...
class PySendPulse:
    ...
    def __send_request(self, path, method="GET", params=None, use_token=True):
        ...
        ilogger.debug("sendp=>__send_request method: {} url: '{}' with parameters: {}".format(method, url, params))
        ...
        if response.status_code == 401 and self.__refresh_token == 0:
            ...
            try:
                ...
                ilogger.debug("sendp=>Request response: {}".format(response.json(), ))
            except:
    ...
    def get_addressbook_variables(self, id):
        """ Get variables from an addressbook

        @param id: unsigned int addressbook ID
        @return: dictionary with response message
        """
        logging.info("Function call: get_addressbook_variables: '{}'".format(id, ))

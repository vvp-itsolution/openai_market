...
from integration_utils.vendors.telegram.error import BadRequest, TimedOut, Unauthorized, RetryAfter
from integration_utils.vendors.telegram.error import ChatMigrated
...
class AbstractBot(models.Model):
    ...
    class HandledUpdate:
        def __init__(
                ...
                is_photo=False, is_document=False, is_chat_migration=False, t_thread=None
        ):
            """
    ...
    def handle_updates(self, fail_silently=False):
        ...
        for up in updates:
            ...
            if self.THREAD_CLASS:
                # Не обязательно для попытки обратной совместимости #TODO убрать в будущем
                t_thread = self.THREAD_CLASS.from_update(up, self, t_chat)
            else:
                t_thread = None
            t_chat.participants.get_or_create(user=t_user)

            handled_update = self.HandledUpdate(update=up, t_user=t_user, t_chat=t_chat, t_thread=t_thread)
            if up.message:
                if up.message.text is not None:
                    command, param = get_command_from_message(up.message, self.username, bot_to_check=self)
                    if command:
                        if self.on_command(up.message, t_user, t_chat, command, param, t_thread):
                            commands_handled += 1
                        ...
                        self.on_message(up.message, t_user, t_chat, t_thread)
                ...
                self.on_query_callback(handled_update)
    ...
    def on_message(self, message, t_user, t_chat, t_thread=None):
        """
    ...
    def on_command(self, message, t_user, t_chat, command, param, t_thread=None):
        """
        ...
        try:
            if command_is_class_method:
                ...
                handler(message, t_user, t_chat, param, t_thread)
            else:
                # нашли команду в отдельном файле в папке self.COMMAND_FUNCTIONS_PATH
                handler(self, message, t_user, t_chat, param, t_thread)
    ...
    def on_query_callback(self, handled_update: HandledUpdate):
        data = handled_update.update.callback_query.data

        if isinstance(data, str) and data.startswith('/'):
            split = data.split(' ', maxsplit=1)
            command, param = split if len(split) == 2 else (split[0], '')
            try:
                return self.on_command(handled_update.update.callback_query.message, handled_update.t_user, handled_update.t_chat, command, param, handled_update.t_thread)
    ...
    def send(self, send_type, chat_id, text, pinned=False, parse_mode=None, reply_markup=None, reply_to_message_id=None,
            disable_notification=False, fail_silently=False, api_kwargs=None, message_thread_id=None):

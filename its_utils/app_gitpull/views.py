...
from its_utils.app_gitpull import gitpull_settings, make_system_checks
...
def do_gitpull(request):
    ...
    if conf['TEST_BEFORE_TOUCH_RESTART']:
        ...
        test_output = u'{}\n\nPATH_TO_PTYHON = {}'.format(
            to_string(test_output), os.path.join(sys.base_prefix, "bin/python"),
        )
...
def view_gitpull(request):
    if request.method == 'GET':
        if not settings.CHECK_SETTINGS_INCLUDED:
            raise PermissionDenied('В файле settings не подключен its_utils.base_django_settings.check_settings')
